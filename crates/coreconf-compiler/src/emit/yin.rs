...
pub fn emit_yin(bundle: &CompiledSchemaBundle) -> String {
    ...
    let mut out = format!(r#"<module name="{}">"#, module.name);
    for path in top_level_paths(bundle, &module.name) {
        render_yin_node(&mut out, path, bundle, 1);
    }
    out.push_str("</module>");
    out
}
...
fn render_yin_node(out: &mut String, path: &str, bundle: &CompiledSchemaBundle, indent: usize) {
    ...
    out.push_str(&format!("{spaces}<{keyword} name=\"{name}\">\n"));
    for must in &node.must {
        out.push_str(&format!("{spaces}  <must value=\"{must}\"/>\n"));
    }
    if let Some(when) = &node.when {
        out.push_str(&format!("{spaces}  <when value=\"{when}\"/>\n"));
    }
    for child in &node.children {
        render_yin_node(out, child, bundle, indent + 1);
    }
    out.push_str(&format!("{spaces}</{keyword}>\n"));
}
